...
def test_gpt5_uses_azure_when_config_exists(monkeypatch: pytest.MonkeyPatch):
    async def fake_azure(message: str, history: list[dict], memory_prompt: str = "", timeout_seconds: float = 25.0):
        del message, history, timeout_seconds
        return f"azure-ok:{memory_prompt}"
    ...
    reply = asyncio.run(
        providers.generate_reply(model="gpt-5", message="hello", history=[], memory_prompt="name=Ali")
    )
    assert reply == "azure-ok:name=Ali"
